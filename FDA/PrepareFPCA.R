...
for (i in 1:n_items) {
    ...
    if (nrow(sample)==0) {
        cat(paste("no f0 detected in region of interest in file", data$filename[i], '\n'))
        next
    }
    # indexing by counter rather than item in data table, of which we may have just skipped a row
    counter <- counter + 1
    stats = data.frame(id = counter, dur_f0 = tail(sample$time,1)-head(sample$time,1), len_f0 = nrow(sample))
    f0_values <- rbind(f0_values, stats)
    sample$id <- counter
...
mean_dur_f0 = mean(f0_values$dur_f0)
min_dur_f0 = min(f0_values$dur_f0)

# GCV for smoothing

n_knots_vec <- seq(4,trunc(median(f0_values$len_f0)/2),4) # explore from 4 knots up to around half the number of samples
...
for (k in n_knots_vec) {
    for (l in loglam_vec) {
        ...
        for (id in sampled_ids) {
            ...
            t_norm <- (all_data$f0_norm[all_data$id==id] / f0_values$dur_f0[f0_values$id==id]) * mean_dur_f0
            bSpline <- tryCatch({
...
cat(paste(all_data$filename[all_data$id==id][1]))